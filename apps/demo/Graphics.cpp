...
void graphics_draw(Graphic::Painter &painter, Recti screen, float time)
{
    ...
    for (int x = 0; x < screen.width(); x++)
    {
        for (int y = 0; y < screen.height(); y++)
        {
            ...
            painter.plot(screen.position() + Vec2i(x, y), Graphic::Color::from_rgb_byte(pixel, pixel, pixel));
        }
    }
    ...
    painter.draw_rectangle(Recti(75, 75, 100, 100), Graphic::Colors::WHITE);
    painter.fill_rectangle(Recti(100, 100, 100, 100), Graphic::Colors::RED.with_alpha(0.5));
    painter.fill_rectangle(Recti(125, 125, 100, 100), Graphic::Colors::GREEN.with_alpha(0.5));
    painter.fill_rectangle_rounded(Recti(150, 150, 100, 100), 32, Graphic::Colors::BLUE.with_alpha(0.5));
