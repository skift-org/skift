...
void Cover::paint(Painter &painter, const WidgetMetrics &metrics, const Recti &)
{
    painter.blit_no_alpha(*_backdrop, _backdrop->bound(), _backdrop->bound().cover(metrics.bound));

    auto cover_bound = Recti{0, 0, 256, 256}.centered_within(metrics.bound);
