...
#include <libgraphic/Rasterizer.h>
...
static int nsvgCmpEdge(const NSVGedge *a, const NSVGedge *b)
{
    if (a->y0 < b->y0)
        return -1;
    if (a->y0 > b->y0)
...
static void nsvgSortEdges(NSVGrasterizer *r)
{
    for (int i = 0; i < r->nedges - 1; i++)
    {
        for (int j = i + 1; j < r->nedges; j++)
        {
            if (nsvgCmpEdge(&r->edges[i], &r->edges[j]) > 0)
            {
                __swap(r->edges[i], r->edges[j]);
            }
        }
    }
}
