export module Hjert.Api:syscalls;
...
export Res<> _now(Instant* i) {
    return _syscall(Syscall::NOW, (Arg)i);
}

export Res<> _log(char const* msg, usize len) {
...
export Res<> _drop(Cap cap) {
    return _syscall(Syscall::DROP, cap.raw());
}

export Res<> _pledge(Cap cap, Flags<Pledge> pledges) {
    return _syscall(Syscall::PLEDGE, cap.raw(), (Arg)pledges.raw());
}

export Res<> _dup(Cap node, Cap* dst, Cap src) {
    return _syscall(Syscall::DUP, node.raw(), (Arg)dst, src.raw());
}

export Res<> _start(Cap cap, usize ip, usize sp, Args const* args) {
...
export Res<> _in(Cap cap, IoLen len, usize port, Arg* val) {
    return _syscall(Syscall::IN, cap.raw(), (Arg)len, port, (Arg)val);
}

export Res<> _out(Cap cap, IoLen len, usize port, Arg val) {
    return _syscall(Syscall::OUT, cap.raw(), (Arg)len, port, val);
}

export Res<> _send(Cap cap, u8 const* buf, usize bufLen, Cap const* caps, usize capLen) {
