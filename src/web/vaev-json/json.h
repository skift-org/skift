...
inline auto operator""_json(char const *str, usize len) {
    return Vaev::Json::parse({str, len}).unwrap();
}

template <>
struct Karm::Io::Formatter<Vaev::Json::Value> {
    Res<usize> format(Io::TextWriter &writer, Vaev::Json::Value value) {
        Io::Emit emit{writer};
        try$(Vaev::Json::stringify(emit, value));
