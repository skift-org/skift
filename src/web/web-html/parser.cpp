...
AdjustedInsertionLocation apropriatePlaceForInsertingANode(Parser &b) {

    // 1. If there was an override target specified, then let target be
    //    the override target.

    //    Otherwise, let target be the current node.
    auto target = last(b._openElements);

    // 2. Determine the adjusted insertion location using the first
    //    matching steps from the following list:

    // NOSPEC: We don't implement foster parenting, let them die of ungroomed hair
...
static void parseRawTextElement(Parser &b, Token const &t) {
    b._insertAnHtmlElement(createElementFor(t, Namespace::HTML));
...
void Parser::_handleBeforeHead(Token const &t) {
    ...
    // A start tag whose tag name is "head"
        ...
        auto head = createElementFor(t, Namespace::HTML);
    ...
    // Anything else
        ...
        auto head = createElementFor(headToken, Namespace::HTML);
        _insertAnHtmlElement(head);
        _headElement = head;
...
void Parser::_acceptInHead(Token const &t) {
    // A character token that is one of U+0009 CHARACTER TABULATION,
    // U+000A LINE FEED (LF), U+000C FORM FEED (FF),
    // U+000D CARRIAGE RETURN (CR), or U+0020 SPACE
    if (t.type == Token::CHARACTER and
    ...
    // A start tag whose tag name is one of: "base", "basefont", "bgsound", "link"
        ...
        _insertAnHtmlElement(createElementFor(t, Namespace::HTML));
    ...
    // A start tag whose tag name is "script"
        ...
        [[maybe_unused]] auto adjustedInsertionLocation = apropriatePlaceForInsertingANode(*this);
